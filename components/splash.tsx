...
export function Splash() {
  ...
  return (
    <section className="relative flex w-full flex-1 flex-col overflow-hidden px-6 pt-[calc(env(safe-area-inset-top)+1.5rem)] pb-[calc(env(safe-area-inset-bottom)+2.25rem)] [@media(max-height:480px)]:pt-4 [@media(max-height:480px)]:pb-5 sm:px-10">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute top-1/2 left-1/2 h-[34rem] w-[52rem] max-w-[170vw] -translate-x-1/2 -translate-y-1/2 rounded-full bg-violet-500/[0.08] dark:bg-violet-500/[0.16] blur-3xl"
      />
      <motion.div
        variants={group}
